...
int main()
{
  ...
  loadShaderNew shaderGrassOpaque("vertex_shader_blending.glsl",
                                  "fragment_shader_blending_opaque.glsl");
  loadShaderNew shaderGrassAlpha("vertex_shader_blending.glsl",
                                 "fragment_shader_blending_alpha.glsl");
  ...
  imgDir     = assetDir + dirPadding + img;
  imgDirChar = imgDir.data();
  imgLoader.loadImgs(imgDirChar, grassTexture, true);
  ...
  shaderGrassOpaque.use();
  shaderGrassOpaque.setInt("texture1", 0);
  shaderGrassAlpha.use();
  shaderGrassAlpha.setInt("texture1", 0);
  ...
  while (!glfwWindowShouldClose(window))
  {
    ...
    shaderGrassOpaque.use();
    shaderGrassOpaque.setMat4("view", view);
    shaderGrassOpaque.setMat4("projection", projection);
    // glActiveTexture(GL_TEXTURE0);
    ...
    for (unsigned int i = 0; i < vegetation.size(); i++)
    {
      ...
      shaderGrassOpaque.setMat4("model", model);
      glDrawArrays(GL_TRIANGLES, 0, 6);
    }
    ...
    glDepthMask(0x00);
    shaderGrassAlpha.use();
    shaderGrassAlpha.setMat4("view", view);
    shaderGrassAlpha.setMat4("projection", projection);
    // glActiveTexture(GL_TEXTURE0);
    ...
    for (unsigned int i = 0; i < vegetation.size(); i++)
    {
      model = glm::mat4();
      model = glm::translate(model, vegetation[i]);
      shaderGrassAlpha.setMat4("model", model);
      glDrawArrays(GL_TRIANGLES, 0, 6);
    }
